...
def cleaning_sine_tracks(track_freq, min_track_length=3):
    ...
    for t in range(n_tracks):  # iterate over all tracks
        ...
        track_begs = (
            np.nonzero(
                (track_freqs[: n_frames - 1] <= 0)  # beginning of track contours
                & (track_freqs[1:] > 0)
            )[0]
            + 1
        )
        ...
        track_ends = (
            np.nonzero(
                (track_freqs[: n_frames - 1] > 0)  # end of track contours
                & (track_freqs[1:] <= 0)
            )[0]
            + 1
        )
...
def f0_twm(peak_freq, peak_mag, max_error_f0, min_f0, max_f0, f0_th=0):
    ...
    if min_f0 < 0:
        raise ValueError(
            f"Minimum fundamental frequency min_f0={min_f0} smaller than 0"
        )

    if max_f0 >= 10000:
        raise ValueError(
            f"Maximum fundamental frequency max_f0={max_f0} bigger than 10000Hz"
        )

    if peak_freq.size < 3 and f0_th == 0:
        return 0

    f0_cand = np.argwhere((peak_freq > min_f0) & (peak_freq < max_f0))[
        :, 0
    ...
    if f0_th > 0:  # if stable f0 in previous frame
        shortlist = np.argwhere(np.abs(f0_cand_freq - f0_th) < f0_th / 2.0)[
            :, 0
        ...
        if (max_cand not in shortlist) and (
            max_cfd > (f0_th / 4)
    ...
    f0, f0_error = twm_peaks(
        peak_freq, peak_mag, f0_cand_freq
    )  # call the TWM function with peak candidates

    if (
        f0 > 0 and f0_error < max_error_f0
...
def peak_interp(mag_x, phase_x, peak_loc):
    ...
    ip_loc = peak_loc + 0.5 * (left_val - right_val) / (
        left_val - 2 * val + right_val
    )  # center of parabola
    ip_mag = val - 0.25 * (left_val - right_val) * (
        ip_loc - peak_loc
    )  # magnitude of peaks
    ip_phase = np.interp(
        ip_loc, np.arange(0, phase_x.size), phase_x
...
def dft_analysis(x, w, fft_size):
    ...
    pos_fft_size = (
        fft_size // 2
    ) + 1  # size of positive spectrum, it includes sample 0
    ...
    fft_buffer[:half_window_round] = audio_w[
        half_window_floor:
    ]  # zero-phase window in fft_buffer
    fft_buffer[-half_window_floor:] = audio_w[:half_window_floor]
    audio_fft = fft(fft_buffer)
    abs_audio_fft = abs(
        audio_fft[:pos_fft_size]
    )  # compute absolute value of positive side
    abs_audio_fft[abs_audio_fft < np.finfo(float).eps] = np.finfo(
        float
    ).eps  # if zeros add epsilon to handle log
    mag_audio_fft = 20 * np.log10(
        abs_audio_fft
    ...
    phase_audio_fft = np.unwrap(
        np.angle(audio_fft[:pos_fft_size])
...
def gen_bh_lobe(x):
    ...
    for m in range(0, 4):  # iterate over the four sincs to sum
        y += (
            consts[m] / 2 * (sinc(f - df * m, fft_size) + sinc(f + df * m, fft_size))
...
def gen_spec_sines(ip_loc, ip_mag, ip_phase, fft_size, sr=44100):
    ...
    for i in range(0, ip_loc.size):  # generate all sine spectral lobes
        ...
        lb = np.arange(
            bin_remainder - 4, bin_remainder + 5
        )  # main lobe (real value) bins to read
        lobe_mag = gen_bh_lobe(lb) * 10 ** (
            ip_mag[i] / 20
        ...
        for m in range(0, 9):
            if b[m] < 0:  # peak lobe crosses DC bin
                ...
                b[m] == 0 or b[m] == half_fft_size
            ):  # peak lobe in the limits of the spectrum
                output_spectrum[b[m]] += lobe_mag[m] * np.exp(
                    1j * ip_phase[i]
                ) + lobe_mag[m] * np.exp(-1j * ip_phase[i])
            else:  # peak lobe in positive freq. range
                output_spectrum[b[m]] += lobe_mag[m] * np.exp(1j * ip_phase[i])
        # fill the negative part of the spectrum
        output_spectrum[half_fft_size + 1 :] = output_spectrum[
            half_fft_size - 1 : 0 : -1
        ].conjugate()
...
def stochastic_model_analysis(audio, fft_size, hop_size, stoch_factor):
    ...
    if (
        pos_fft_size * stoch_factor < 3
    ):  # raise exception if decimation factor too small
    ...
    while p_in <= p_end:
        xw = (
            audio[p_in - half_fft_size : p_in + half_fft_size] * w
        )  # window the input sound
        audio_fft = fft(xw)  # compute FFT
        audio_fft_mag = 20 * np.log10(
            abs(audio_fft[:pos_fft_size])
        )  # magnitude spectrum of positive frequencies
        # decimate the mag spectrum
        output_mag = signal.resample(
            np.maximum(-200, audio_fft_mag), int(stoch_factor * pos_fft_size)
        )
...
def sine_subtraction(x, fft_size, hop_size, sfreq, smag, sphase, sr):
    ...
    x = np.append(
        np.zeros(half_fft_size), x
    )  # add zeros at beginning to center first window at sample 0
    x = np.append(
        x, np.zeros(half_fft_size)
    )  # add zeros at the end to analyze last sample
    ...
    sw[half_fft_size - hop_size : half_fft_size + hop_size] = (
        signal.windows.triang(2 * hop_size)
        / w[half_fft_size - hop_size : half_fft_size + hop_size]
    )
    ...
    for frame in range(num_frames):
        ...
        cleaned_spectrum = (
            spectrum - generated_sines
        )  # subtract sines from original spectrum
    ...
    xr = np.delete(
        xr, range(half_fft_size)
    )  # delete half of first window which was added in stftAnal
    xr = np.delete(
        xr, range(xr.size - half_fft_size, xr.size)
    )  # delete half of last window which was added in stftAnal
    return xr


def stochastic_residual_analysis(
    x, fft_size, hop_size, sine_freq, sine_mag, sine_phases, sr, stoch_factor
):
    """
    ...
    for frame in range(num_frames):
        ...
        # generate spec sines
        y_harm = gen_spec_sines(
            fft_size * sine_freq[frame, :] / sr,
            sine_mag[frame, :],
            sine_phases[frame, :],
            fft_size,
        )
        ...
        # decimate the mag spectrum
        x_fft_res_mag_env = signal.resample(
            np.maximum(-200, x_fft_res_mag), x_fft_res_mag.size * stoch_factor
        )
        stoch_env = None
        if frame == 0:
...
def harmonic_detection(
    ...
    if (
        len(harm_freq_prev) == 0
    ):  # if no incoming harmonic tracks initialize to harmonic series
        harm_freq_prev = harm_freq

    while (
        (f0 > 0) and (harm_idx < num_harm) and (harm_freq[harm_idx] < sr / 2)
    ):  # find harmonic peaks
        pei = np.argmin(abs(peak_freq - harm_freq[harm_idx]))  # closest peak
        dev1 = abs(
            peak_freq[pei] - harm_freq[harm_idx]
        )  # deviation from perfect harmonic
        # deviation from previous frame
        dev2 = (
            abs(peak_freq[pei] - harm_freq_prev[harm_idx])
            if harm_freq_prev[harm_idx] > 0
            else sr
        )
...
def harmonic_model_analysis(
    ...
    x = np.append(
        np.zeros(half_fft_floor), x
    )  # add zeros at beginning to center first window at sample 0
    x = np.append(
        x, np.zeros(half_fft_floor)
    )  # add zeros at the end to analyze last sample
    ...
    while p_in <= p_end:
        ...
        ip_loc, ip_mag, ip_phase = peak_interp(
            mag_fft, phase_fft, peak_loc
        )  # refine peak values
        ip_freq = sr * ip_loc / fft_size  # convert locations to Hz
        f0_track = f0_twm(ip_freq, ip_mag, f0_eth, min_f0, max_f0, f0_stable)  # find f0
        if ((f0_stable == 0) and (f0_track > 0)) or (
            (f0_stable > 0) and (np.abs(f0_stable - f0_track) < f0_stable / 5.0)
        ):
            f0_stable = (
                f0_track  # consider a stable f0 if it is close to the previous one
            )
        else:
            f0_stable = 0
        harm_freq, harm_mag, harm_phase = harmonic_detection(
            ip_freq,
            ip_mag,
            ip_phase,
            f0_track,
...
def fft_extractor(
    ...
    h_freq, h_mag, h_phase = harmonic_model_analysis(
        x,
        sr,
        w,
        fft_size,
